...
function search($keyword, $activePage)
{
  // pagination
  // konfigurasi
  ...
  $dataAmountPerPage = 2;
  // $dataAmount = count(get_data("SELECT * FROM students WHERE name LIKE '%$keyword%'"));

  // halaman = 2, awal data = 2
  // halaman = 3, awal data = 4

  $initialData = ($dataAmountPerPage * intval($activePage)) - $dataAmountPerPage;


  $students = "SELECT *, students.id
  FROM ((students 
...
function pagination($activePage, $dataAmountPerPage)
{
  // pagination
  // konfigurasi
  // jumlah halaman = total data / data perhalaman
...
function pageAmount($dataAmount, $dataAmountPerPage) {
  return ceil(count(get_data($dataAmount)) / $dataAmountPerPage);
}
